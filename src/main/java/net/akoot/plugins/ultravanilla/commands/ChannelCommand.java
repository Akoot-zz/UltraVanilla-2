...
public class ChannelCommand extends UltraCommand {
    ...
    protected boolean onCommand() {
        if (sender instanceof Player) {
            ...
            // editchannel
            else if ("editchannel".equals(name)) {
                if (args.length >= 2) {
            ...
            // leacechannel
            else if ("leavechannel".equals(name)) {
                if (args.length == 1) {
                    String id = args[0];
                    Channel channel = uv.getChannel(id);
                    if (channel != null) {
            ...
            // createchannel
            else if ("createchannel".equals(name)) {
                if (args.length == 1) {
                    String id = args[0];
            ...
            // deletechannel
            else if ("deletechannel".equals(name)) {
                if (args.length == 1) {
                    String id = args[0];
                    if (uv.hasChannel(id)) {
                        uv.removeChannel(id);
            ...
            // listchannels
            else if ("listchannels".equals(name)) {
                if (args.length == 0) {
            ...
            // hidechannel
            else if ("hidechannel".equals(name)) {
                if (args.length == 1) {
                    String id = args[0];
                    Channel channel = uv.getChannel(id);
                    if (channel != null) {
                        if (channel.isMember(user.getPlayer())) {
                            if (user.getHiddenChannels().contains(channel)) {
                                user.getPlayer().sendMessage("Channel with id \"" + id + "\" is already hidden!");
                            } else {
                                user.hideChannel(id);
                                user.getPlayer().sendMessage("Hid channel \"" + id + "\"");
                            }
                        } else {
                            user.getPlayer().sendMessage("You do not belong to a channel called \"" + id + "\"");
                        }
            ...
            // showchannel
            else if ("showchannel".equals(name)) {
                if (args.length == 1) {
                    String id = args[0];
                    Channel channel = uv.getChannel(id);
                    if (channel != null) {
                        if (channel.isMember(user.getPlayer())) {
                            if (!user.getHiddenChannels().contains(channel)) {
                                user.getPlayer().sendMessage("Channel with id \"" + id + "\" is already visible!");
                            } else {
                                user.showChannel(id);
                                user.getPlayer().sendMessage("Made channel \"" + id + "\" visible");
                            }
                        } else {
                            user.getPlayer().sendMessage("You do not belong to a channel called \"" + id + "\"");
                        }
